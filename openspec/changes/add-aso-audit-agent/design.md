...
| **Skill: `aso-audit`** | A Markdown skill file containing the full audit rubric, the ten dimensions and weights, the output format (Score Card, Quick Wins, High-Impact, Strategic, Competitor Comparison), and the explicit "what's not visible" policy. Vendored and refined from `Eronred/aso-skills` (see D11). Loaded by the agent at audit time. This is the only skill the service ships; before/after copy for metadata and screenshot recommendations is generated by the agent directly. |
...
### D12. Single-skill adoption; explicitly skip Appeeky

We vendor exactly one skill file from `aso-skills`:

1. `aso-audit` — primary, always loaded by the audit agent.

We do **not** vendor `metadata-optimization`, `screenshot-optimization`, `keyword-research`, `competitor-analysis`, `aso-router`, or any of the other 25-plus skills. Reasons:
...
- We initially vendored `metadata-optimization` and `screenshot-optimization` as on-demand sub-skills. We removed them because the audit agent's main instructions already cover before/after generation for those recommendation types adequately — the extra skill files were redundant prose and added load-the-wrong-skill variance for no measurable quality gain.
...
**Trade-off:** A single skill file means metadata and screenshot recommendations don't carry the upstream's templated copy frameworks verbatim. Mitigation: the agent's instructions enumerate the structural requirements (cite evidence, supply before/after for text changes) and the LLM has enough general ASO knowledge to fill in the framework details.
...
- Firecrawl v2 HTTP API (`POST https://api.firecrawl.dev/v2/scrape`) called directly via `fetch` — no SDK dependency.
- `zod@^4.4.0` (with `z.toJSONSchema()` for the Firecrawl extraction-schema conversion)
...
| Firecrawl integration | The `@mendable/firecrawl-js` SDK was dropped in favour of calling the v2 HTTP API directly (`POST https://api.firecrawl.dev/v2/scrape` with Bearer auth). This eliminates an external dependency, makes the request body inspectable in one place (`scrape/scraper/firecrawl.ts`), and avoids the SDK's bundled Zod 3 conflict with our Zod 4 schemas. Documented 402/429/5xx responses and upstream 404s (via `data.metadata.statusCode`) are mapped to the typed `ScraperError` shape; `data.json` carries the extracted payload; `data.metadata.ogImage` carries the og:image used to recover the canonical app icon. |
| Zod 4 ↔ Firecrawl JSON Schema | Firecrawl's JSON format expects a plain JSON Schema. We convert our Zod 4 extraction schema at the request boundary with the native `z.toJSONSchema(schema, { target: 'draft-7', unrepresentable: 'any' })`, then re-validate the response against the original Zod schema so downstream code gets fully-typed data. |
