...
- Adopt the audit rubric and supporting prose from [`Eronred/aso-skills`](https://github.com/Eronred/aso-skills) (MIT) — the same open-source project the brief itself was adapted from. Vendor a single skill file: `aso-audit`, refined to match the brief's exact rubric table and our visibility policy. Do **not** integrate the Appeeky MCP server — keep our Firecrawl scraper as the data source.
